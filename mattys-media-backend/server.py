...
async def login(credentials: UserLogin):
    ...
    return AuthResponse(
        token=token,
        user=UserResponse(id=user_doc['id'], email=user_doc['email'], name=user_doc['name'])
    )
...
async def create_or_update_cart(cart_data: CartCreate, current_user: dict = Depends(get_current_user)):
...
async def generate_whatsapp_message(current_user: dict = Depends(get_current_user)):
    ...
    for item in cart_doc['items']:
        custom_text = f" | Custom: {item.get('custom_text')}" if item.get('custom_text') else ""
        message_lines.append(f"• {item['product_name']} x{item['quantity']} - ${item['price']:.2f}{custom_text}")
